/**
Start-up code for ARM Cortex-M4 microcontroller on tiva-c launchpad board using
tm4c123gh6pm microcontroller.

Strat-up code for most other processors written in assemblly, because SP not 
set up yet and .data not copy from ROM to RAM, and .bss isn't cleared ye. but 
ARM Cortex-M designed to reduce the need for low-level assembly.
*/

///contain fult hadler and IRQ prototypes 
#include "tm4c_cmsis"

/**
CSTACK$$Limit is inker symbol refore to the start of the stack. I found this
name in desassembly during debugging. the details description in HELP -->
content --> Compiler reference --> using C --> Compiler and linker reference
*/
...
/// Create The Vector table in address 0x00
/**
-the name "__vector_table" come from map file in section "ENTRY LIST" to replace
the defult one that created by IAR toolchain.
-@ ".intvec" to place the array in .intvec section to replace the defult. this 
synthetics special for IAR toolchain, no C standard fro this command so it is 
depend on the toolchain.
-const to force the vector table in ROM instead of RAM.
*/
const int __vector_table [] @ ".intvec" = 
{
    ///filling the array with it's value according to data sheet
    (int)&CSTACK$$Limit, ///start of the stack 
...
/**
because the stack may be corrupted, so I want the handler doesn't use the stack 
to avoid infinite loop of push into stack cause hard fault and hard fault 
handler push into the stack and initiate hard fault.
To avoid accessing the stack in fault handler, IAR help --> Extended keyword 
--> Summary of extended keywords --> __stackless 
A function declared __stackless violates the calling convention in such a way 
that it is not possible to return from it. However, the compiler cannot reliably
detect if the function returns and will not issue an error if it does.
*/
...
/**
The weak alias means that if the symbol remains undefined till the end of the 
linking proecess the provided alias will be used.
*/
